...
> ⚠️ **Estás en la rama `inicio` (plantilla de partida).** Los módulos del juego
> están como esqueleto con comentarios `TODO`: tu trabajo es completarlos
> siguiendo las guías de `docs/`. Si ejecutas `python main.py` antes de empezar,
> verás un mensaje indicando qué falta implementar (es normal).
>
> La **solución completa** vive en la rama `main`. Para empezar tu curso, crea tu
> rama de trabajo a partir de esta: `git checkout -b tetris-nombre-apellido inicio`.
